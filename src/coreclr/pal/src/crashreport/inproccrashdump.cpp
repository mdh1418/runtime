...
// Registered callbacks from VM — set during initialization, read from signal handler
...
{
    g_resolveMethodCallback = callback;
}
...
// Callback for writing stack frames to logcat/console
static void LogcatFrameCallback(uint64_t ip, const char* methodName, const char* className,
    const char* moduleName, uint32_t nativeOffset, uint32_t token, void* ctx)
{
    ...
    {
        len = snprintf(line, sizeof(line), "   at %s.%s() + 0x%x [%s] token:0x%08x\n",
            className, methodName ? methodName : "?", nativeOffset, moduleName ? moduleName : "?", token);
    }
    else if (methodName != NULL)
    {
        len = snprintf(line, sizeof(line), "   at %s() + 0x%x [%s] token:0x%08x\n",
            methodName, nativeOffset, moduleName ? moduleName : "?", token);
    }
    else
    {
        len = snprintf(line, sizeof(line), "   at 0x%llx [%s]\n",
            (unsigned long long)ip, moduleName ? moduleName : "?");
    }
    if (len > 0) write(STDERR_FILENO, line, len);
}

// Callback for writing stack frames to JSON
static void JsonFrameCallback(uint64_t ip, const char* methodName, const char* className,
    const char* moduleName, uint32_t nativeOffset, uint32_t token, void* ctx)
{
