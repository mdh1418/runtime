...
SignalSafeConsoleWriter::AppendStr(const char* s)
{
    ...
    while (*s != '\0' && m_pos + 1 < sizeof(m_buffer))
    {
        m_buffer[m_pos++] = *s++;
    }
}
...
{
    char buf[SignalSafeJsonWriter::MAX_HEX_FORMAT_BUFFER_SIZE];
    SignalSafeJsonWriter::FormatHexValue(buf, sizeof(buf), v);
...
{
    char buf[SignalSafeJsonWriter::MAX_UNSIGNED_DECIMAL_BUFFER_SIZE];
    SignalSafeJsonWriter::FormatUnsignedDecimal(buf, sizeof(buf), v);
    AppendStr(buf);
}
...
{
    char buf[SignalSafeJsonWriter::MAX_SIGNED_DECIMAL_BUFFER_SIZE];
    SignalSafeJsonWriter::FormatSignedDecimal(buf, sizeof(buf), v);
    AppendStr(buf);
}
